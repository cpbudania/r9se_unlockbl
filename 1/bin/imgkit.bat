...
for /f "tokens=2 delims== " %%a in ('type tmp\imgkit-magiskpatch\util_functions.sh ^| find "MAGISK_VER="') do set magiskver_show=%%a
...
set origbootsize=
for /f "tokens=5 delims= " %%i in ('busybox.exe ls -l %bootpath%') do set origbootsize=%%i
if "%origbootsize%"=="" ECHOC {%c_e%}获取%bootpath%大小失败{%c_i%}{\n}& call log %logger% F 获取%bootpath%大小失败 & goto FATAL
set patchedbootsize=
for /f "tokens=5 delims= " %%i in ('busybox.exe ls -l tmp\imgkit-magiskpatch\boot_new.img') do set patchedbootsize=%%i
