...
adb.exe shell ./sgdisk -l %write__adbpush__filepath% %diskpath% 1>>%logfile% 2>&1 || ECHOC {%c_e%}恢复分区表失败. {%c_h%}按任意键重试...{%c_i%}{\n}&& call log %logger% E 恢复分区表失败&& pause>nul && ECHO.重试... && goto RECPARTABLE-1
...
call framwork adbpre sgdisk
adb.exe shell ./sgdisk -b ./bff_sgdiskpartable.bak %diskpath% 1>>%logfile% 2>&1 || ECHOC {%c_e%}备份分区表失败. {%c_h%}按任意键重试...{%c_i%}{\n}&& call log %logger% E 备份分区表失败&& pause>nul && ECHO.重试... && goto BAKPARTABLE-1
...
adb.exe shell ./sgdisk -n %parnum%:%parstart_sec%:%parend_sec% %diskpath% 1>>%logfile% 2>&1 || ECHOC {%c_e%}创建分区失败. {%c_h%}按任意键重试...{%c_i%}{\n}&& call log %logger% E 创建分区失败&& pause>nul && ECHO.重试... && goto MK-4
:MK-5
call log %logger% I 开始命名分区
adb.exe shell ./sgdisk -c %parnum%:%parname% %diskpath% 1>>%logfile% 2>&1 || ECHOC {%c_e%}命名分区失败. {%c_h%}按任意键重试...{%c_i%}{\n}&& call log %logger% E 命名分区失败&& pause>nul && ECHO.重试... && goto MK-5
:MK-6
call log %logger% I 开始设置分区类型
adb.exe shell ./sgdisk -t %parnum%:%partype% %diskpath% 1>>%logfile% 2>&1 || ECHOC {%c_e%}设置分区类型失败. {%c_h%}按任意键重试...{%c_i%}{\n}&& call log %logger% E 设置分区类型失败&& pause>nul && ECHO.重试... && goto MK-6
...
adb.exe shell ./sgdisk -d %parnum% %diskpath% 1>>%logfile% 2>&1 || ECHOC {%c_e%}删除分区失败. {%c_h%}按任意键重试...{%c_i%}{\n}&& call log %logger% E 删除分区失败&& pause>nul && ECHO.重试... && goto RM-2
::call reboot recovery recovery rechk 3
...
adb.exe shell ./sgdisk --resize-table=%target% %diskpath% 1>>%logfile% 2>&1 || ECHOC {%c_e%}设置最大分区数失败. {%c_h%}按任意键重试...{%c_i%}{\n}&& call log %logger% E 设置最大分区数失败&& pause>nul && ECHO.重试... && goto SETMAXPARNUM-1
